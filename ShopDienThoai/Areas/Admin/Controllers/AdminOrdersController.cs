...
namespace ShopDienThoai.Areas.Admin.Controllers
{
    ...
    public class AdminOrdersController : Controller
    {
        ...
        public AdminOrdersController(DienthoaiDBContext context, INotyfService notifyService)
        {
            _context = context;
        ...
        public IActionResult Index(int? page)
        {
            ...
            var lsOrders = _context.Orders.Include(o => o.Customer).Include(o => o.TransactStatus)
                .AsNoTracking().OrderByDescending(x => x.OrderDate);
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var Chitietdonhang = _context.OrderDetails.Include(x=>x.Product).AsNoTracking().Where(x => x.OrderId == order.OrderId)
        ...
        public async Task<IActionResult> ChangeStatus(int? id)
        {
            ...
            //Cái này k rõ
            var order = await _context.Orders.AsNoTracking().Include(x=>x.Customer).FirstOrDefaultAsync(x=>x.Paid);
            if (order == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> ChangeStatus(int id, [Bind("OrderId,CustomerId,OrderDate,ShipDate,TransactStatusId,Deleted,Paid,PaymentDate,TotalMoney,PaymentId,Note,Address,LocationId,District,Ward")] Order order)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var donhang = await _context.Orders.AsNoTracking().Include(x => x.Customer)
                        .FirstOrDefaultAsync(x=>x.Paid);
                    if(donhang != null)
                    {
